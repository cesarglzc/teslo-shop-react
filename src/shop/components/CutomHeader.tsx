...
export const CustomHeader = () => {
  ...
  const [searchParams, setSearchParams] = useSearchParams() // Parametros opcionales

  const { authStatus, isAdmin, logout } = useAuthStore()

  const { gender } = useParams() // SON parametros obligatorios
          ...
          <div className="flex items-center space-x-4">
            <div className="hidden md:flex items-center space-x-2">
              <div className="relative">
                ...
                <Input ref={inputRef} placeholder="Buscar productos..." className="pl-9 w-64 h-9 bg-white" 
                onKeyDown={handleSearch} defaultValue={query}/>
              </div>
            </div>
